...
def get_output_categories(ib_bw_output: str) -> dict:
    category_info = {
        'dev_line': None, 'qp_num_line': None, 'conn_type_line': None, 'srq_line': None, 
...
def extract_value(pattern: str, line: str):
    match = re.search(pattern, line)
    return match.group(1).strip() if match else None

def get_local_remote_qpn(qpn_line_list: list, is_local: bool) -> list:
    qpn_list = []
    for line in qpn_line_list:
        pattern = r"local address:.*QPN\s+(0x[0-9a-fA-F]+)" if is_local else r"remote address:.*QPN\s+(0x[0-9a-fA-F]+)"
        match = re.search(pattern, line)
        if match:
            qpn_list.append(match.group(1))
    return qpn_list

def get_bw_result(bw_result_line_list: list) -> dict:
    bw_res = {}
...
def get_perftest_result(perftest_output: str) -> dict:
    ...
    perftest_res = {
        'device': extract_value(r".*Device\s*:\s*(\S+)", cate_info.get('dev_line')),
        'qp_num': int(extract_value(r"Number of qps\s*:\s*(\d+)", cate_info.get('qp_num_line'))),
        'conn_type': extract_value(r"Connection type\s*:\s*(\S+)", cate_info.get('conn_type_line')),
        'srq': extract_value(r"Using SRQ\s*:\s*(.+)", cate_info.get('srq_line')),
        'rx_depth': int(extract_value(r"RX depth\s*:\s*(\d+)", cate_info.get('rx_depth_line'))),
        'mtu': int(extract_value(r"Mtu\s*:\s*(\d+)\[B\]", cate_info.get('mtu_line'))),
        'rdma_cm_qps': extract_value(r"rdma_cm QPs\s*:\s*(\S+)", cate_info.get('rdma_cm_qps_line')),
        'data_method': extract_value(r"Data ex. method\s*:\s*(\S+)", cate_info.get('data_method_line')),
        'local_qpn_list': get_local_remote_qpn(cate_info.get('local_qpn_line_list'), is_local=True),
        'remote_qpn_list': get_local_remote_qpn(cate_info.get('remote_qpn_line_list'), is_local=False),
