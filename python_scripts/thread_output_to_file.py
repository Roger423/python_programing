...
class CommandThread(threading.Thread):
    def __init__(self, host_dev: Host, command: str, output_file_path: str=None, live_print: bool=False):
        ...
        self.total_bytes = 0
        self.output_file = output_file_path or f'/home/cmd_output_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
    ...
    def _buffer_output(self, text):
        ...
        if self.total_bytes >= self.max_queue_size_bytes:
            self._flush_to_file()
    ...
    def get_output(self, timeout=None):
        ...
        try:
            with open(self.output_file, 'r') as f:
                return ''.join(f.readlines())  # 每行读入，拼接成完整输出
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # th = CommandThread(ht, cmd, output_file='test_output.txt')
